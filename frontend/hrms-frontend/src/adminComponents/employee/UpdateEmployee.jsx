...
export default function UpdateEmployee() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${ADMIN_BASE}/employees/${id}`).then(r => r.json()),
      fetch(`${ADMIN_BASE}/departments`).then(r => r.json()),
      fetch(`${ADMIN_BASE}/designations`).then(r => r.json()),
      ...
      if (e.department?.id) {
        fetch(`${ADMIN_BASE}/employees/filter?departmentId=${e.department.id}`)
          .then(r => r.json())
          .then(obj => setManagers(obj.data ?? []));
      }
  ...
  useEffect(() => {
    ...
    fetch(`${ADMIN_BASE}/employees/filter?departmentId=${selectedDeptId}`)
      .then(r => r.json())
      .then(obj => setManagers(obj.data ?? []))
      .catch(() => setManagers([]));
  }, [selectedDeptId]);
