...
export default function FetchBranches() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Fall back to search endpoint if search term present
        if (filters.search) {
          url = `${ADMIN_BASE}/branches?search=${encodeURIComponent(filters.search)}&`;
          if (filters.sortBy)        url += `sortBy=${filters.sortBy}&`;
          if (filters.sortDirection) url += `sortDirection=${filters.sortDirection}&`;
        }
  ...
  if (branches === null) return <div className="hrms-content"><div className="d-flex align-items-center gap-2"><span className="spinner-border spinner-border-sm"></span> Loading...</div></div>;
