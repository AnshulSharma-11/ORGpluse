...
function UploadPanel({ employeeId, onUploaded }) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      let res = await fetch(`${EMPLOYEE_BASE(employeeId)}/documents`, {
        method: 'POST',
        body: fd,
      });
...
function DocumentCard({ doc, employeeId, onDeleted }) {
  ...
  let downloadUrl = `${EMPLOYEE_BASE(employeeId)}/documents/${doc.id}/download`;
...
export default function MyDocuments() {
  ...
  const loadDocs = useCallback(function loadDocs() {
    setLoading(true);
    fetch(`${EMPLOYEE_BASE(employeeId)}/documents`)
      .then(r => r.json())
      .then(obj => setDocs(obj.data ?? []))
